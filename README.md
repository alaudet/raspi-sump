...
An unofficial port is maintained on the [Arch User Repository](https://aur.archlinux.org/packages/raspi-sump), which hosts the build file and its required dependencies for running Raspi-Sump on Arch Linux via the [unofficial ARM64 port](https://ports.archlinux.page/aarch64/) or on [Arch Linux ARM](https://archlinuxarm.org/). This port is community-maintained and not officially supported here.
